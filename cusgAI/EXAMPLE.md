...
int main()
{
	...
	//R.__agents[0].file_load("actor.txt");
	//R.__agents[1].file_load("critic.txt");
	//R.__agents[2].file_load("critic2.txt");
	vector<float> s = { 0,0, 3,3 };
	vector<float> ss = { 0,0, 3, 3 };
	vector<string> action_list = { "up","    stop","  down","  left","  right" };
	int iter = 0;
	while (iter++ < 20000)
	{
		...
		if(R.__agents.size() > 1)
			printf("--------[Q1]---------\n"), show_vector(R.Q_S_W_A(R.__agents[1], exp.s, R.__act_list));
		if (R.__agents.size() > 2)
			printf("--------[Q2]---------\n"), show_vector(R.Q_S_W_A(R.__agents[2], exp.s, R.__act_list));
		if(R.LOSS_X.size() > 0)
			printf("--------[UP]---------\n"), show_vector(R.LOSS_X), show_vector(R.LOSS_Y);
		printf("--------[alpha]---------\n");
		printf("%f\n", R.alpha);
