...
class modelLearn(threading.Thread):
    ...
    def __init__(self, neural, out, trg):
        super().__init__()
        self.neural = neural
        self.out = out
        self.trg = trg
    ...
    def run(self):
        self.neural.learn_start(out_val=self.out, target_val=self.trg)

    def join(self, *args, **kwargs) -> np.ndarray:
        super().join(*args, **kwargs)
        return self.neural.get_update_layer


class modelRun(threading.Thread):

    def __init__(self, model, exp, gamma, qn, tqn):
        """
        Args:
            model(function): model for forward
            exp(np.ndarray): [s,a,r,s',t]
    ...
    def run(self):
        self.trg, self.out, self.err = self.model(self.exp, self.gamma, self.qn, self.tqn)

    def join(self, *args, **kwargs) -> list:
        super().join(*args, **kwargs)
        return [self.trg, self.out, self.err]
...
class openRL:
    ...
    def RL_ACTION(self, s, agent, act_sz, epsilon) -> int:
        """
        Args:
    ...
    def RL_PROCESS_MERGE(self, initial_state, show=True) -> bool:
        ...
        for self.__ep in range(self.__max_epoch):
            ...
            for self.__it in range(self.__max_iter):  # increase the interation
                ...
                a = self.RL_ACTION(s=s, agent=self.qn, act_sz=3, epsilon=action_probability)  # get a (action)
